/**
 * @file    os_config.h
 * @brief   RP2040 系统目标工程裁剪配置 —— 完整基础版（含 Shell + FatFs + MSC +
 *          Periph + 超频/多核/IPC），**无展示功能**（OS_CFG_SHOW_DEMO=0）。
 *
 *  与 rp2040demo 的唯一区别：无 OLED FPS / 呼吸 LED / GP15 PWM 展示任务
 *  （demo_show.c）。其余全部 Shell 命令（fs / msc / ovclk / mcore / ipc /
 *  bootscript / vtest / reboot / bootlog ...）与功能完全一致。
 *
...
#define OS_CFG_VFS                    0   /* 虚拟文件系统接口 —— 直接用 FatFs，不经过 VFS 抽象 */
#define OS_CFG_FATFS                  1   /* Elm FatFs（FAT16，MSC U 盘 + 本机文件目录命令）*/
#define OS_CFG_LOADER                 0   /* 用户程序加载器（需 VFS=1）    */
#define OS_CFG_DEMO_APP               1   /* 启动演示应用（提供 shell 任务） */
#define OS_CFG_SHOW_DEMO              0   /* RP2040 系统版：无展示任务（OLED FPS / 呼吸 LED / GP15 PWM）。Demo 版置 1 */
...
#define OS_CFG_IDLE_STACK_SIZE        4096 /* 空闲任务栈 (字节)。v2.7.1: 1024B 崩、
                                            ...
                                            * 长打印(replay/ovclk)频繁 yield→idle 溢出
                                            * →idle PC=0 HardFault。取 4096 留 2 倍余量+嵌套帧。 */
