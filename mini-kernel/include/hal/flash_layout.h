/**
 * @file    flash_layout.h
 * @brief   v2.2 W25Q16JV (2MiB) Flash 分区定义 — 固件 + MSC U 盘 + Bootscript
 *
 ...
 *  │  1,040,384 Bytes = 2032 × 512B/sector                         │
 *  │  0x00100000 → 0x001FDFFF                                      │
 *  │                                                               │
 *  ├───────────────────────────────────────────────────────────────┤  0x00100000 (1 MiB)
 *  │                                                               │
...
#define FLASH_LAYOUT_MSC_SECTORS       (2032u)
#define FLASH_LAYOUT_MSC_BYTES         (FLASH_LAYOUT_MSC_SECTORS * FLASH_LAYOUT_MSC_SECTOR_BYTES)
                                                              /* = 1,040,384 B */
#define FLASH_LAYOUT_MSC_END_EXCLUSIVE (FLASH_LAYOUT_MSC_OFFSET + FLASH_LAYOUT_MSC_BYTES)
                                                              /* = 0x001FE000，恰好是 SEC_A */
...
/* ---------- 编译期断言：检查所有边界严丝合缝 ---------- */
#if (FLASH_LAYOUT_MSC_END_EXCLUSIVE != FLASH_LAYOUT_BOOTSCRIPT_A)
#error "MSC data region end must touch bootscript SEC_A start exactly — no gap, no overlap"
#endif
